...
defineFeature(feature, test => {
  ...
  test('The player notifies a good answer for a card from the second partition', ({ given, and, when, then }) => {
    ...
    playerSteps["when the player notifies a (good|wrong) answer for the current reviewed flashcard for the box \"(.*)\""](when, depsContainer);

    boxSteps["then the current score for the box \"(.*)\" should be (d*)"](then, depsContainer);

    sessionDeckSteps["and the currently reviewed flashcard for the box \"(.*)\" should now be at the end of the partition (d)"](() => currentlyReviewedFlashcard, and, depsContainer);

    sessionDeckSteps["and the flashcard question to review for the box \"(.*)\" should be \"(.*)\""](and, depsContainer)
  });
