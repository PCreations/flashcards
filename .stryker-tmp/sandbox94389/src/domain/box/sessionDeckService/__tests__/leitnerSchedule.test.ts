...
const partition = (number: PartitionNumber) => ({
  shouldBeIncludedForSessions(...sessions: SessionNumber[]) {
    Array(64)
      .fill(0)
      .map((_, index) => ({
        session: index + 1,
        partitions: leitnerSchedule((index + 1) as SessionNumber),
      }))
      .forEach(({ session, partitions }: { session: SessionNumber; partitions: PartitionNumber[] }) => {
        test(`session ${session} should ${
          sessions.some(currentSession => currentSession === session) ? 'contain' : 'not contain'
        } partition ${number}`, () => {
          sessions.some(currentSession => currentSession === session)
