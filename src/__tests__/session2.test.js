...
describe('given 3 boxes [c1,c3][c2,c4,c5][] and previous session was session 1', () => {
  describe('when starting a session', () => {
    ...
    describe('and c1 and is correctly answered but not c3', () => {
      test('then the boxes should be [c3][c2,c4,c5,c1][] and the previous session set to 2', async () => {
        ...
        expect(box3).toEqual([]);
        expect(await flashCards.getPreviousSessionNumber()).toEqual(2);
      });
    });
  });
});
